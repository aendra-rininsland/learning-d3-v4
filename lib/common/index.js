...
export const descendantsDarker = (d, color, invert = false, dk = 5, idProp = 'itemLabel') => {
  const depth = d.depth;
  let parent = d;

  while (parent.depth > 1) parent = parent.parent;

  const colorVal = d3.color(color(parent.data[idProp]));
  const k = depth / dk;
  return invert ? colorVal.brighter(k) : colorVal.darker(k);
};
