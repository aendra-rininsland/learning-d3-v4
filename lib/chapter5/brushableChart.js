...
console.dir(interactivePrisonChart);
brushablePrisonPopulationChart.margin = {
  top: 50,
  left: 50,
  bottom: 50,
  right: 50,
...
(async (enabled) => {
  if (!enabled) return;
  await brushablePrisonPopulationChart.resolveData();
  await brushablePrisonPopulationChart.init();
  brushablePrisonPopulationChart.addUIElements();
})(true);
