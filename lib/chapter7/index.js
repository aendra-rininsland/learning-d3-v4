...
westerosChart.histogram = function histogram(_data) {
  const data = _data.data.sort((a, b) =>
    (a.death.season !== b.death.season ?
    a.death.season - b.death.season :
    a.death.episode - b.death.episode)
  );
  console.dir(data);
  ...
  const allEpisodes = d3.range(1, totalSeasons + 1).reduce((episodes, s) =>
    episodes.concat(d3.range(1, episodesPerSeason + 1).map(e => `S${s}E${e}`)), []);
  ...
  const histogram = d3.histogram()
    .value(d => `S${d.death.season}E${d.death.episode}`)
    .thresholds(x.domain());
...
westerosChart.stream = function Stream({ data }) {
  const episodesPerSeason = 10;
  const totalSeasons = 6;
